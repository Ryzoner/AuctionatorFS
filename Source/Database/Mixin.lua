...
function Auctionator.DatabaseMixin:SetPrice(dbKey, buyoutPrice, available)
  ...
  -- Firestorm patch: deserialize CBOR-encoded entries on access
  if type(self.db[dbKey]) == "string" then
    local success, data
    if C_EncodingUtil then
      success, data = pcall(C_EncodingUtil.DeserializeCBOR, self.db[dbKey])
    else
      local LibCBOR = LibStub("LibCBOR-1.0")
      success, data = pcall(LibCBOR.Deserialize, LibCBOR, self.db[dbKey])
    end
...
function Auctionator.DatabaseMixin:GetPriceAge(dbKey)
  -- Firestorm patch: deserialize CBOR-encoded entries on access
  if self.db[dbKey] ~= nil and type(self.db[dbKey]) == "string" then
    local success, data
    if C_EncodingUtil then
      success, data = pcall(C_EncodingUtil.DeserializeCBOR, self.db[dbKey])
    else
